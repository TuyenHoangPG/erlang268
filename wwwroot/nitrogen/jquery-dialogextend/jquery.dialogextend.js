(function() {
  ...
  $.widget("ui.dialogExtend", {
    ...
    _initState: function () {
      if (this.options.state == "minimize")
        this.minimize();
      else if (this.options.state == "maximize")
        this.maximize();
      else if (this.options.state == "collapse")
        this.collapse();
    },
    _setContainment: function() {
      if($(this.element[0]).dialog("option").draggable) {
        var containment;
        if(this.options.containment == 'body') {
          if (!$("#dialog_extend_area_containment").length) {
            var containment = $("<div id='dialog_extend_area_containment'></div>").appendTo('body');
            containment.css({
              "width": "100%",
              "height": "100%",
              "position": "fixed",
            ...
            containment = $("#dialog_extend_area_containment")[0];
          }
        } else {
          containment = this.options.containment;
        }
        $(this.element[0]).dialog("widget").draggable("option", "containment", containment);
        $(this.element[0]).dialog("widget").draggable("option", "scroll", false);
      }
    },
...
(function() {
  ...
  $.extend(true, $.ui.dialogExtend.prototype, {
    ...
    maximize: function() {
      var newHeight, newWidth, position, containment;

      if (this.options.containment != '' && this.options.containment != 'body') {
          containment = this.options.containment;
          newHeight = $(containment).height() - 11;
          newWidth = $(containment).width() - 11;
          position = 'absolute';
      } else {
          containment = window;
          newHeight = $(containment).height() - 11;
      ...
      this._saveSnapshot();
      if ($(this.element[0]).dialog("option", "draggable")) {
        this.original_config_draggable_option = $(this.element[0]).dialog("widget").draggable("option");
        $(this.element[0]).dialog("widget").draggable("option", "handle", null).find(".ui-dialog-draggable-handle").css("cursor", "text").end();
      }
      $(this.element[0]).dialog("widget").css("position", position).find(".ui-dialog-content").show().dialog("widget").find(".ui-dialog-buttonpane").show().end().find(".ui-dialog-content").dialog("option", {
        "resizable": false,
        "draggable": false,
        "height": newHeight,
        "width": newWidth,
    ...
    _restore_maximized: function() {
      ...
      if ($(this.element[0]).dialog("option", "draggable")) {
        $(this.element[0]).dialog("widget").draggable(this.original_config_draggable_option);
...
(function() {
  ...
  $.extend(true, $.ui.dialogExtend.prototype, {
    ...
    minimize: function() {
      ...
      newWidth = 200;
        var containerId = 'dialog-extend-fixed-container';
        var position = 'fixed';
        if(this.options.containment != 'body') {
          containerId += ('-' + $(this.options.containment)[0].id);
          position = 'relative';
        }
      if ($("#" + containerId).length) {
        fixedContainer = $("#" + containerId);
      } else {
        fixedContainer = $('<div id=' + containerId + '></div>').appendTo(this.options.containment);
        fixedContainer.css({
          "position": position,
          "bottom": 1,
          "left": 1,
      ...
      dialogcontrols.css({
        ...
        if(this.options.containment != 'body') {
          var width = $(this.options.containment).width();
          var number = fixedContainer.children().length;
          var numberEachRow = Math.floor(width / newWidth);
          var row = Math.ceil(number / numberEachRow);
          var top = $(this.options.containment).height() - (dialogcontrols.outerHeight() + 2) * row;
          this._dialogcontrols = dialogcontrols;
    ...
    _minimize_restoreOnClose: function() {
      var $this = $(this).dialogExtend("instance");
      if($this.options.containment != 'body') {
        var width = $($this.options.containment).width();
        var number = $this._fixedContainer.children().length - 1;
        var numberEachRow = Math.floor(width / $this._newWidth);
        var row = Math.ceil(number / numberEachRow);
        var top = $($this.options.containment).height() - ($this._dialogcontrols.outerHeight() + 2) * row;
