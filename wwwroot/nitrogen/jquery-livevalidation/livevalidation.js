...
// This is list of input types that
// the getElementType function should return LiveValidation.TEXT
LiveValidation.LIST_INPUT_TYPE =
    ["TEXT", "EMAIL", "NUMBER", "DATE", "TIME", "DATETIME", "DATETIME-LOCAL", "MONTH", "WEEK", "SEARCH", "TEL", "URL", "COLOR"];
...
// ANHLQ: add a feature to display error messages by mouseover/tooltip
LiveValidation.prototype = {
    ...
    initialize : function(D, C) {
        ...
        this.displayMode = B.displayMode;
    ...
    remove : function(B, D) {
        // オブジェクトの比較がうまくできていないため、remove関数が動作しませんでした。比較する関数（objectEquals）を追加して使います。
        ...
        for (var C = 0, A = this.validations.length; C < A; C++) {
            if (this.validations[C].type == B) {
                if (objectEquals(this.validations[C].params, D)) {
                    E = true;
                    break;
                }
            }
    ...
    getElementType : function() {
        ...
        case (this.element.nodeName.toUpperCase() == "INPUT" && (LiveValidation.LIST_INPUT_TYPE
                .indexOf(this.element.type.toUpperCase()) != -1)):
        ...
        case (this.element.nodeName.toUpperCase() == "FIELDSET"):
    ...
    validateElement : function(A, C) {
        ...
        var K = C.allowBlank || false;
        var U = C.allowUndefined || false;
        var T = /^\s+$/.test(D);
        if ((T && K) || (D === '' && U))
            return true;
        if (!K || !U)
    ...
    insertTooltip : function() {
        ...
        if ((this.displayMessageWhenEmpty && (this.elementType == LiveValidation.CHECKBOX || this.element.value == ""))
                || this.element.value != "") {
            jQuery(this.element).tooltipster({
                content: this.message,
                restoration: 'previous',
                theme: 'LV_invalid_tooltip',
                position: 'right'
            });
        }
    },
    ...
    removeTooltip : function() {
        try {
            jQuery(this.element).tooltipster("destroy");
            jQuery(this.element).attr('title', '');
        } catch (Err) {
        }
    },
